...
class TaskController {
    ...
    public static function create() {
        ...
        foreach ($notifyIds as $uid) {
            ...
            if ($uid > 0 && $uid !== Auth::userId()) {
                ...
                $html = "<h3>New task assigned</h3><p><b>{$actor['name']}</b> assigned you a new task:</p><p style='font-size:16px'><b>{$data['title']}</b></p><p><a href='" . APP_URL . "'>View in VGo →</a></p>";
            ...
            foreach ($toAdd as $aid) {
                ...
                if ($aid > 0) {
                    ...
                    if ($aid !== Auth::userId()) {
                        ...
                        $html = "<h3>New task assigned</h3><p><b>{$actor['name']}</b> assigned you a task:</p><p style='font-size:16px'><b>{$task['title']}</b></p><p><a href='" . APP_URL . "'>View in VGo →</a></p>";
    ...
    public static function show($id) {
        ...
        jsonResponse(['task' => [
            ...
            'source_module' => $taskRow['source_module'] ?? null,
            'crm_lead_id' => !empty($taskRow['crm_lead_id']) ? (int)$taskRow['crm_lead_id'] : null,
    ...
    public static function myTasks() {
        ...
        $result = array_map(fn($t) => [
            ...
            'crm_lead_id' => !empty($t['crm_lead_id']) ? (int)$t['crm_lead_id'] : null,
            'status' => $t['status'],
